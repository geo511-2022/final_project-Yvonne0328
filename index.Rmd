...
SCORE  <- c(sum(dfbefore$score), sum(dfduring$score), sum(dfafter$score),sum(dfafter2$score),sum(dfafter3$score),
            sum(dfafter4$score))
timeframe <- c("-1", "0", "1", "2", "3", "4")
names <- c("before", "during", "3D-after", "6D-after", "9D-after", "12D-after")
trend <- data.frame(names, timeframe, SCORE)

ggplot(trend, aes(x= factor(names,level = c("before", "during", "3D-after", "6D-after", "9D-after", "12D-after")), SCORE, group = 1))+
